...
from util import util  # Import the util module for utility functions
import torch  # Import PyTorch library

# Define the BaseOptions class for handling options and configurations
class BaseOptions():
    def __init__(self):
        self.parser = argparse.ArgumentParser()  # Create an ArgumentParser object
        self.initialized = False  # Flag to check if options are initialized
    ...
    def initialize(self):
        ...
        self.initialized = True  # Mark options as initialized

    def parse(self, save=True):
        if not self.initialized:  # Initialize options if not already done
            self.initialize()
        self.opt = self.parser.parse_args()  # Parse command-line arguments
        self.opt.isTrain = self.isTrain  # Set training mode

        str_ids = self.opt.gpu_ids.split(',')  # Split GPU IDs into a list
        self.opt.gpu_ids = []
        for str_id in str_ids:
            id = int(str_id)  # Convert GPU ID to integer
            if id >= 0:
        ...
        args = vars(self.opt)  # Convert arguments to a dictionary
        ...
        util.mkdirs(expr_dir)  # Create directories
        if save and not self.opt.continue_train:  # Save options only if not continuing training
            ...
            with open(file_name, 'wt') as opt_file:
                ...
                for k, v in sorted(args.items()):
                    opt_file.write('%s: %s\n' % (str(k), str(v)))
                opt_file.write('-------------- End ----------------\n')
        return self.opt  # Return parsed options
